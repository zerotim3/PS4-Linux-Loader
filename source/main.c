...
int kpayload(struct thread *td, struct kpayload_args* args){
	...
	int (*set_nclk_mem_spd)(int val) = (void *)(kernel_base + 0x244FA0);
	int (*set_pstate)(int val) = (void *)(kernel_base + 0x497EA0);
	int (*set_gpu_freq)(int cu, unsigned int freq) = (void *)(kernel_base + 0x4A02E0);
	int (*update_vddnp)(unsigned int cu) = (void *)(kernel_base + 0x4A06C0); //!!
	int (*set_cu_power_gate)(unsigned int cu) = (void *)(kernel_base + 0x4A0850);
	...
	kernel_ptr[0x99E9E] = 3; //4.55 pstate when shutdown
